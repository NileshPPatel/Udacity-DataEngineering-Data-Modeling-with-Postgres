...
songplay_table_create = ("""CREATE TABLE IF NOT EXISTS songplays (\
    songplay_id int PRIMARY KEY, start_time bigint NOT NULL, user_id int NOT NULL, level varchar,\
...
user_table_insert = ("""INSERT INTO users (user_id, first_name, last_name, gender,\
    level) VALUES (%s, %s, %s, %s, %s) ON CONFLICT (user_id) DO UPDATE SET level\
    = EXCLUDED.level """)
...
song_select = ("""SELECT s.song_id, a.artist_id FROM artists a JOIN\
    songs s ON a.artist_id = s.artist_id WHERE s.title = %s AND a.name = %s\
    AND s.duration = %s """)
